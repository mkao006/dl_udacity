...
# Please go to http://www.usgs.gov/visual-id/credit_usgs.html for further
# information
...
def show_path(path, starting_city, w=12, h=8):
    """Plot a TSP path overlaid on a map of the US States & their
    capitals.
...
class TravelingSalesmanProblem:
    ...
    def get_value(self):
        ...
        dy = np.diff([y for x, y in city_coords])
        path_distance = sum(np.sqrt(dx**2 + dy**2))
        return path_distance * -1.0


# Construct an instance of the TravelingSalesmanProblem
test_cities = [('DC', (11, 1)), ('SF', (0, 0)),
               ('PHX', (2, -3)), ('LA', (0, -4))]
...
# The start/end point is indicated with a yellow star
print(capitals_list[:num_cities])
show_path(capitals_tsp.coords, starting_city)

# set the decay rate and initial temperature parameters, then run
# simulated annealing to solve the TSP
alpha = 0.95
temperature = 1e6
