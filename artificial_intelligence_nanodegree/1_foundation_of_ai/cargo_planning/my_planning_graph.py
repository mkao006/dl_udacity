...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        #
        # The literals to be added are basically the effected nodes as
        # a result of the actions in the previous level.
        for action_node in self.a_levels[level - 1]:
            for effect_node in action_node.effnodes:
                # 2. connect the nodes
                #
                # for example, every A node in the previous level has
                # a list of S nodes in effnodes that represent the
                # effect produced by the action.  These literals will
                # all be part of the new S level.  Since we are
                # working with sets, they may be "added" to the set
                # without fear of duplication.  However, it is
                # important to then correctly create and connect all
                # of the new S nodes as children of all the A nodes
                # that could produce them, and likewise add the A
                # nodes to the parent sets of the S nodes
